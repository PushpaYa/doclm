...
def create_input_id(df,image_path):
    ...
    for index ,rows in df.iterrows():
        ...
        for k in range(12):
                ...
                if nbrs_index==99999 or df.iloc[nbrs_index]["text"]=="":
                    text=[" "]
                    box=[[0,0,0,0]]
                    word_labels=["O"]
                    encoding = processor(image ,text,boxes = box,word_labels=word_labels,padding="max_length", truncation=True)
                    
                else:
                    #print("index",nbrs_index)
                    #print("text",df.iloc[nbrs_index]["text"])
                    #print(df.iloc[nbrs_index]["text"])
                    #print(df.iloc[nbrs_index]['box'])
                    #print(df.iloc[nbrs_index]['label'])
                    encoding = processor(image,[str(df.iloc[nbrs_index]["text"])],boxes = [df.iloc[nbrs_index]['box']],word_labels=[df.iloc[nbrs_index]['label']],padding="max_length", truncation=True)
...
def generate_nbrs(filepath):
    ...
    for guid, file in enumerate(sorted(os.listdir(ann_dir))):
        ...
        df[["x0","y0","x1","y1"]]=df['box'].astype('str').str.extractall('(\d+)').unstack().fillna('').astype(int)
        df["text"]=df["text"]
        ...
        boundry_y=1.0*sum(avg_diff_y1)/len(avg_diff_y1)
        ...
        #df['neighbors'] = pd.Series(create_input_id(df,img_path))
        df['neighbors'] = pd.Series(create_id_to_txt(df))
        ##neighbors and neighbours are diff 
        df=df[['label', 'text', 'box','neighbors']]
...
#generate_nbrs(path+'/testing_data/')
